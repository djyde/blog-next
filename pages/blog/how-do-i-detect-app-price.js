...
export default () => (
  <Blog>
    ...
    <h3 id="app">获取 APP 信息</h3>

    <p>首先我们应该怎么通过 APP 的 URL 获取 APP 的信息，比如 Shadowmatic 的 URL 是 https://itunes.apple.com/cn/app/shadowmatic/id775888026?mt=8 。在我观察页面 DOM 结构的时候，我发现这种 APP 页面是有 <a href="https://www.wikiwand.com/en/Schema_matching">semantic schema</a> 的。并且提供的内容非常多，包括应用的名字、截屏、评分、价格等等。</p>

    <p>于是二话不说写了个 <a href="https://github.com/djyde/Catchem/blob/master/utils/appstore.js">解析 APP 页面信息的方法</a>，这个方法解析后的整合到的信息是：</p>

    <pre><code className="javascript language-javascript">{`{
  ...
  aggregateRating: '4.88354395 份评分',
  ratingValue: '4.88354',
  reviewCount: '395 份评分'
}`}
</code></pre>
    ...
    <pre><code className="javascript language-javascript">{`// https://github.com/djyde/Catchem/blob/master/models%2Fapp.js#L141-L147
...
`}</code></pre>
    ...
    <h3 id="hook">发送微信消息的 hook</h3>

    <p>由于我经常用微信，如果 Catchem 可以及时把降价信息推送到我微信的话那是极好的。于是我写了一个给自己发送微信消息的 hook，这个 hook 使用测试订阅号的 token。</p>

    <p>hook 相关的代码段：https://github.com/djyde/lean-hook/blob/master/routes%2Fwechat.js#L31-L54</p>
    ...
    <p>整个项目开源在 https://github.com/djyde/Catchem ，大家可以拿来自己用，前提是遵循 MIT License。</p>
